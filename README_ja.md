...
// 1. ベースURL・認証トークンを明示的に使用する、fetch APIを使用したSenderオブジェクトを生成
...
## 独自Senderオブジェクトとコンテキスト値 (Advanced topic)

場合によっては、fetch APIではなく、別のトランスポート実装を使いたい場合があります。
例えば、axiosのようなライブラリや、WebSocketを使用する場合などです。

modestaでは、Senderオブジェクトを差し替えることで、任意のトランスポート層を使用することが出来ます。

また、カスタムのSenderオブジェクトを定義すれば、API呼び出しの際にout-of-band valueとなる「コンテキスト値」を渡すことが出来ます。
  ...
  return {
    ...
    send: async (request, requestValue, accessorOptions) => {
      ...
      const response = await axios.request({
        ...
        // シリアライザでデシリアライズできるようにレスポンスpayloadを未変換のまま取得する
...
- Sender実装は、`send` の第2引数としてリクエスト値を受け取ります。Senderファクトリ内で `modestaSerializeRequestValue(request, requestValue, serializers)` を呼び出してシリアライズします。
- レスポンスのデシリアライズもSenderファクトリの責務です。fetch以外のトランスポートでは、未変換のレスポンスpayloadを取得して `modestaDeserializeResponsePayload()` に渡し、そのデシリアライズ済みの値を `modestaProjectResponse()` に渡します。
  未変換のpayloadは選択されたシリアライザへそのまま渡されるため、`undefined` のような値をどう扱うかはシリアライザが決定します。
  既に fetch 互換の `Response` を扱っている場合は、`modestaReadFetchResponseValue(response, request.responseContentType, serializers, request.responseBodyMetadata)` を `modestaProjectResponse()` の前に使用します。
...
// カスタムシリアライザを指定してSenderオブジェクトを生成
const sender = createFetchSender({
