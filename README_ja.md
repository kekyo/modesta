...
例えば、アクセサ毎に固有の値や、呼び出し毎に固有の値をコンテキスト値として渡して、リクエストヘッダに追加する、などの利用方法が考えられます。

コンテキスト値は2種類存在します:

- アクセサ生成時に指定して、そのアクセサを使用する場合に同じ値を使用する: `TAccessorInterfaceContext`
- アクセサ関数呼び出し時に指定して、APIアクセス毎に異なる値を使用する: `TAccessorContext`
...
const sender: AccessorSenderWithContext<InterfaceContext, Context> = async (
  ...
  const response = await myTransport(request.url, {
    ...
    body: request.body,
    traceId: interfaceContext.traceId,     // InterfaceContextの値を参照
    requestId: options.context.requestId,  // Contextの値を参照
...
// SenderからAPIへのアクセサを生成する
const summaries = create_ListSummaries_accessor(sender, {
  traceId: 'trace-42',  // アクセサ生成時にInterfaceContextを指定する必要がある
});
...
`createFetchSender()` は `TAccessorInterfaceContext` と `TAccessorContext` を常に無視します。
`createFetchSender()` で生成されたsenderを使う限りは、これらのコンテキスト値を明示的に指定する必要はありません（型の制約により出来ません）。

補足: 生成コードを利用する TypeScript プロジェクトでは、`exactOptionalPropertyTypes` を有効にすることを推奨します。これにより、より厳密にコンテキスト値の有無を検査できます。
