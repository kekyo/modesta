...
export const create_GetUser_accessor = (sender: AccessorSender): GetUser => ({
  get: async (args, signal) =>
    sender<GetUser_get_response, undefined>(/* ... */),
});
```
...
const sender: AccessorSender = async (request, signal) => {
